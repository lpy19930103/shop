...
function getSuitInfoService(t) {
    if (null !== t.packResponseList && t.packResponseList.length > 0) {
        ...
        $("#favorable-suit").show(), $("#favorable-suit .mc").html(suit_TPL.tabs.process(t) + suit_TPL.cons.process(t)), $("#favorable-suit .mc").Jtab({event: e >= t.packResponseList.length ? "mouseover" : "click",delay: 300,compatible: !0,currClass: "scurr"}, function(t, e) {
            var i = e.attr("packprice"), s = e.attr("data-cart"), a = e.attr("packlistprice"), n = e.attr("discount"), o = e.find(".fitting-price"), r = e.find(".orign-price"), c = e.find(".fitting-saving"), l = e.find(".infos .p-name a,.infos .btns a"), d = [G.sku];
...
//        , $.ajax({url: "http://club.jd.com/ProductPageService.aspx?method=GetCommentSummaryBySkuId&referenceId=" + t + "&callback=getCommentCount",dataType: "script",cache: !0})
        ...
        var s = $(i), a = e, n = s.find(".suits"), o = s.find(".stab li"), r = s.find(".suits .lh"), c = n.find('li[data-cart="' + a + '"]');
        if ("all" == e) {
...
//        var t = G.sku, e = 6728 == G.cart[0] || 653 == G.cart[1] || 830 == G.cart[1], i = "-672-6864-1105-717-720-718-719-798-", s = RegExp("-" + G.cart[2] + "-").test(i);
//        return !G.isJd && e ? !1 : (655 == G.cart[2] ? $.ajax({url: "http://rs.jd.com/accessorie/newServiceWhite.jsonp?sku=" + G.sku + "&callback=Recommend.cbNewFittings",dataType: "script",cache: !0,scriptCharset: "utf-8"}) : 830 == G.cart[1] ? $.ajax({url: "http://rs.jd.com/accessorie/accessoryByAccessory.jsonp?sku=" + G.sku + "&callback=Recommend.cbNewFittings",dataType: "script",cache: !0,scriptCharset: "utf-8"}) : G.cart[2] > 830 && 835 > G.cart[2] || 5012 === G.cart[2] ? $.ajax({url: "http://misc.360buyimg.com/product/js/2012/fittingsCamera.js?ver=20140424",dataType: "script",cache: !0,success: function() {
//                fittingsCamera.init(pageConfig.product.skuid, pageConfig.product.cart[2], 1)
...
//                fittingsCamera.init(pageConfig.product.skuid, pageConfig.product.cart[2], 2)
//            }}) : 6728 == G.cart[0] && G.isJd ? $.ajax({url: "http://misc.360buyimg.com/product/js/2012/fittingsAuto.js?ver=20140424",dataType: "script",cache: !0}) : $.getJSONP("http://d.360buy.com/fittingInfo/get?skuId=" + t + "&callback=Recommend.cbRecoFittings"), void 0)
    },cbNewFittings: function(t) {
        var e = $("#tab-reco"), i = {accessoryList: []}, s = {tabs: '<div class="tab-cart stab">    <ul>        {if pageConfig.product.cart[1]!=830}            <li id="tab-cart-0" class="fl scurr" data-widget="tab-item" data-cart="\u914d\u4ef6\u9009\u8d2d\u4e2d\u5fc3|3">\u7cbe\u9009\u914d\u4ef6</li>            {for tab in accessoryList}            <li id="tab-cart-${parseInt(tab_index)+1}" class="fl" data-widget="tab-item" data-cart="${tab.thirdName}|${tab.thirdTypeId}">${tab.thirdName}</li>            {/for}        {else}            {for tab in accessoryList}            <li id="tab-cart-${parseInt(tab_index)+1}" class="{if Number(tab_index)==0}scurr{/if} fl" data-widget="tab-item" data-cart="${tab.thirdName}|${tab.thirdTypeId}">${tab.thirdName}</li>            {/for}        {/if}    </ul></div>',cons: '<div id="newFittign-tab"  data-widget="tabs"><div class="master">    <s></s>    <div class="p-img">        <a href="http://item.jd.com/${mainproduct.sku}.html" target="_blank"><img src="${pageConfig.FN_GetImageDomain(mainproduct.sku)}n4/${mainproduct.imageUrl}" height="100" width="100"></a>    </div>    <div class="p-name">        <a href="http://item.jd.com/${mainproduct.sku}.html" target="_blank">${mainproduct.name}</a>    </div>    <div class="p-price"><input id="inp_${mainproduct.sku}_reco" type="checkbox" onclick="return false;" onchange="return false" wmeprice="{if mainproduct.price==""}0.00{else}${mainproduct.price}{/if}" wmaprice="${mainproduct.maPrice}" skuid="${mainproduct.sku}" checked/> ${mainproduct.price}</div></div><div class="suits" style="width:${pageConfig.wideVersion&&pageConfig.compatible?(4*165-40):(3*128)}px;overflow-x:{if G.cart[1]!=830}auto{else}hidden{/if};overflow-y:hidden;">    {for tab in accessoryList}    <ul id="newFitting-${parseInt(tab_index)+1}" data-cart="${tab.thirdName}" style="width:${G.cart[1]==830?165*tab.accessoryList.length:660}px;" class="{if G.cart[1]==830}{if Number(tab_index)!=0}hide{/if}{else}hide{/if} lh" data-widget="tab-content">        {if G.cart[1]==830}            {for list in tab.accessoryList}            <li data-push="${pageConfig.product.cbNewFittings.push(list.sku)}" {if (parseInt(list_index)+1)==tab.accessoryList.length} class="last_item"{/if} onclick=\'${pageConfig.setFittingsClk(G.cart[2], G.sku, list.sku, 50, list_index, "rodGlobalTrack")}\'>                <s></s>                <div class="p-img">                    <a href="http://item.jd.com/${list.sku}.html" target="_blank"><img src="${pageConfig.FN_GetImageDomain(list.sku)}n4/${list.imageUrl}" alt="" height="100" width="100"></a>                </div>                <div class="p-name">                    <a href="http://item.jd.com/${list.sku}.html" target="_blank">${list.name}</a>                </div>                <div class="choose">                    <input type="checkbox" id="inp_${list.sku}" onclick="G.calculatePrice(this, \'#tab-reco\')" wmaprice="${list.maPrice}" wmeprice="${list.price}" skuid="${list.sku}" />                    <label for="inp_${list.sku}" class="p-price">                        <strong class="J-p-${list.sku}"></strong>                    </label>                </div>            </li>            {/for}        {else}            <div class="iloading">\u52a0\u8f7d\u4e2d...</div>        {/if}    </ul>    {/for}</div><div class="infos">    {if pageConfig.product.cart[1]!=830}<div id="more-fitting-link"><a class="hl_link" href="http://rs.jd.com/accessorie/center.html?sku=${G.sku}&thirdTypeId=3" target="_blank">\u8fdb\u5165\u914d\u4ef6\u9009\u8d2d\u4e2d\u5fc3</a><span>&gt;<b></b></span></div>{/if}    <s></s>    <div class="p-name">        <em>\u5df2\u9009\u62e9<span>0</span>\u4e2a\u914d\u4ef6</em>    </div>    <div class="p-price">\u642d&nbsp;&nbsp;\u914d&nbsp;&nbsp;\u4ef7\uff1a        <strong class="res-jdprice">{if mainproduct.price==""}\u6682\u65e0\u62a5\u4ef7{else}\uffe5 ${parseFloat(mainproduct.price).toFixed(2)}{/if}</strong>    </div>    <div class="p-saving">\u53c2&nbsp;&nbsp;\u8003&nbsp;&nbsp;\u4ef7\uff1a        <span class="res-totalprice">\uffe5 ${parseFloat(mainproduct.maPrice).toFixed(2)}</span>    </div>    <div class="btns">        <a class="btn-buy" href="http://jd2008.jd.com/purchase/OrderFlowService.aspx?action=AddSkus&wids=${G.sku}">\u7acb\u5373\u8d2d\u4e70</a>    </div></div></div>',item: '{for item in accessoryList}<li data-push="${pageConfig.product.cbNewFittings.push(item.sku)}" {if (parseInt(item_index)+1)==accessoryList.length} class="last_item"{/if}  onclick=\'${pageConfig.setFittingsClk(G.cart[2], G.sku, item.sku, 34, item_index)};\'>    <s></s>    <div class="p-img">        <a href="http://item.jd.com/${item.sku}.html" target="_blank"><img src="${pageConfig.FN_GetImageDomain(item.sku)}n4/${item.imageUrl}" alt="" height="100" skuidth="100"></a>    </div>    <div class="p-name">        <a href="http://item.jd.com/${item.sku}.html" target="_blank">${item.name}</a>    </div>    <div class="choose">        <input type="checkbox" id="inp_m_${item.sku}" onclick="G.calculatePrice(this, \'#tab-reco\')" wmaprice="${item.maPrice}" wmeprice="${item.price}" skuid="${item.sku}" />        <label for="inp_m_${item.sku}" class="p-price">            <strong class="J-p-${item.sku}"></strong>        </label>    </div>    {if isExtra==true}    <div class="p-more{if (parseInt(item_index)+1)!==accessoryList.length} hide{/if}"><a class="hl_link" href="http://rs.jd.com/accessorie/center.html?sku=${G.sku}&thirdTypeId=${item.thirdType}" target="_blank"></a></div>    {else}    <div class="p-more"><a class="hl_link" href="http://rs.jd.com/accessorie/center.html?sku=${G.sku}&thirdTypeId=${item.thirdType}" target="_blank">\u66f4\u591a${item.thirdName}</a></div>    {/if}</li>{/for}'};
        if (pageConfig.product.cbNewFittings = [pageConfig.product.skuid], pageConfig.setFittingsClk = function(t, e, i, s, a) {
            ...
            $("#newFittign-tab").prepend(s.tabs.process(t)).Jtab({event: "click",compatible: !0,currClass: "scurr"}, function(t, i, a) {
                var n = $("#newFittign-tab .suits"), o = $("#tab-cart-" + a).attr("data-cart").split("|")[1], r = $("#tab-cart-" + a).attr("data-cart").split("|")[0], c = $("#more-fitting-link a");
                if (830 != G.cat[1])
                    if (c.attr("href", "http://rs.jd.com/accessorie/center.html?sku=" + G.sku + "&thirdTypeId=" + o).html("\u8fdb\u5165" + r), n.scrollLeft(0), 0 !== a) {
                        ...
                        window.fetchJSON_fittingExtra = function(t) {
                            ...
                            t.isExtra = !0, t.accessoryList = t.accessoryList.slice(0, i), $("#newFitting-" + a).html(s.item.process(t)).attr("loaded", "1"), $("#newFitting-" + a).find(".hl_link").html("\u66f4\u591a" + $("#tab-cart-" + a).html());
                            try {
...
var Refresh = {init: function() {
        ...
        }, this.url = "", this.typeMap = {t2: "ycbs",t3: "ycbs",t4: "ycbs",t6: "ywbh",t7: "ywbh",t9: "yhdx"}, this.itemTPL = '<div class="item">    {if arguments[2][0]&&arguments[2][1]==0}        <ins class="yb-item-more">\u66f4\u591a</ins>    {/if}    <b></b>    <a class="yb-link" href="http://fuwu.jd.com" target="_blank"><i class="yb-ico"></i></a>    <a href="#none" class="yb-item {if arguments[2][0]&&arguments[2][1]==0} yb-item-first{/if}" id="yb-pid-${platformPid}" data-type="${sortId}" data-sku="${platformPid}" title="${G.showServiceDesc(sortId)}">${sortName} \uffe5${price}</a></div>', this.TPL = '<div class="service-type-yb">    {if r1.length>0}        <div class="yb-item-cart yb-item-cart-r1">            ${JdService.itemTPL.process(r1[0], [r1.length>1, 0])}            <div class="yb-item-hide">                {for item in r1}                    {if item_index!=0}${JdService.itemTPL.process(item, [r1.length>1])}{/if}                {/for}            </div>        </div>    {/if}    {if r2.length>0}        <div class="yb-item-cart yb-item-cart-r2">            ${JdService.itemTPL.process(r2[0], [r2.length>1, 0])}            <div class="yb-item-hide">                {for item in r2}                    {if item_index!=0}${JdService.itemTPL.process(item, [r2.length>1])}{/if}                {/for}            </div>        </div>    {/if}    {if r3.length>0}        <div class="yb-item-cart yb-item-cart-r3">            ${JdService.itemTPL.process(r3[0], [r3.length>1, 0])}            <div class="yb-item-hide">                {for item in r3}                    {if item_index!=0}${JdService.itemTPL.process(item, [r3.length>1])}{/if}                {/for}            </div>        </div>    {/if}    {if r4.length>0}        <div class="yb-item-cart yb-item-cart-r4">            ${JdService.itemTPL.process(r4[0], [r4.length>1, 0])}            <div class="yb-item-hide">                {for item in r4}                    {if item_index!=0}${JdService.itemTPL.process(item, [r4.length>1])}{/if}                {/for}            </div>        </div>    {/if}    {if r5.length>0}        <div class="yb-item-cart yb-item-cart-r5">            ${JdService.itemTPL.process(r5[0], [r5.length>1, 0])}            <div class="yb-item-hide">                {for item in r5}                    {if item_index!=0}${JdService.itemTPL.process(item, [r5.length>1])}{/if}                {/for}            </div>        </div>    {/if}    <div class="yb-item-cart yb-item-detail"><a class="hl_blue" target="_blank" href="http://sale.jd.com/act/1pChm0RcanBPYXIS.html?erpad_source=erpad">\u8be6\u60c5</a></div></div>', this.yb_TPL = '        <div class="service-type-yb">            {for data in list}            <div class="yb-item-cart ${data.cName}">                <div class="item">                    {if data.items.length > 1}<ins class="yb-item-more">\u66f4\u591a</ins>{/if}                    <b></b>                    <a class="yb-link" href="http://fuwu.jd.com" target="_blank"><i class="yb-ico"></i></a>                    <a href="#none" class="yb-item {if data.items.length > 1} yb-item-first{/if}" id="yb-pid-${data.items[0].platformPid}" data-type="${data.items[0].sortId}" data-sku="${data.items[0].platformPid}" title="${G.showServiceDesc(data.items[0].sortId)}">${data.items[0].sortName} \uffe5${data.items[0].price}</a>                </div>                {if data.items.length > 1}                <div class="yb-item-hide">                    {for yb in data.items}                    {if Number(yb_index)>0}                    <div class="item">                        <b></b>                        <a class="yb-link" href="http://fuwu.jd.com" target="_blank"><i class="yb-ico"></i></a>                        <a href="#none" class="yb-item" id="yb-pid-${yb.platformPid}" data-type="${yb.sortId}" data-sku="${yb.platformPid}" title="${G.showServiceDesc(yb.sortId)}">${yb.sortName} \uffe5${yb.price}</a>                    </div>                    {/if}                    {/for}                </div>                {/if}            </div>            {/for}            <div class="yb-item-cart yb-item-detail">                <a class="hl_blue" target="_blank" href="http://sale.jd.com/act/1pChm0RcanBPYXIS.html?erpad_source=erpad">\u8be6\u60c5</a>            </div>        </div>', this.get()
    },bindEvent: function() {
        var t = this;
        this.obj.find(".service-type-yb .item a").bind("click", function() {
            var e = $(this), i = e.attr("data-sku"), s = (e.attr("data-type"), e.parent(".item")), a = e.parents(".yb-item-cart");
...
//            }})
        ...
        for (e = 0; a > e; e++)
            i = RegExp("-" + t[e].sortId + "-"), i.test("-2-3-4-44-49-") ? (n.list[0] || (n.list[0] = {}, n.list[0].items = [], n.list[0].cName = "yb-item-cart-r0"), n.list[0].items.push(t[e])) : i.test("-6-7-41-47--") ? (n.list[1] || (n.list[1] = {}, n.list[1].items = [], n.list[1].cName = "yb-item-cart-r1"), n.list[1].items.push(t[e])) : i.test("-9-48-60-") ? (n.list[2] || (n.list[2] = {}, n.list[2].items = [], n.list[2].cName = "yb-item-cart-r2"), n.list[2].items.push(t[e])) : i.test("-11-50-") ? (n.list[3] || (n.list[3] = {}, n.list[3].items = [], n.list[3].cName = "yb-item-cart-r3"), n.list[3].items.push(t[e])) : i.test("-15-16-17-18-45-46-61-63-") ? (n.list[4] || (n.list[4] = {}, n.list[4].items = [], n.list[4].cName = "yb-item-cart-r4"), n.list[4].items.push(t[e])) : i.test("-20-21-22-76-77-85-86-87-") ? (n.list[5] || (n.list[5] = {}, n.list[5].items = [], n.list[5].cName = "yb-item-cart-r5"), n.list[5].items.push(t[e])) : i.test("-27-28-31-32-67-68-69-70-78-79-") ? (n.list[6] || (n.list[6] = {}, n.list[6].items = [], n.list[6].cName = "yb-item-cart-r6"), n.list[6].items.push(t[e])) : i.test("-42-64-65-66-71-72-73-74-80-81-") ? (n.list[7] || (n.list[7] = {}, n.list[7].items = [], n.list[7].cName = "yb-item-cart-r7"), n.list[7].items.push(t[e])) : i.test("-52-53-54-57-58-59-75-82-83-84-") ? (n.list[8] || (n.list[8] = {}, n.list[8].items = [], n.list[8].cName = "yb-item-cart-r8"), n.list[8].items.push(t[e])) : (n.list[9] || (n.list[9] = {}, n.list[9].items = [], n.list[9].cName = "yb-item-cart-r9"), n.list[9].items.push(t[e]));
        if (s.list.length > 0) {
            this.obj.html(this.yb_TPL.process(n)).parent().show(), pageConfig.product.hasYbInfo = !0;
            var o = this.obj.find(".yb-item-cart");
            o.hover(function() {
